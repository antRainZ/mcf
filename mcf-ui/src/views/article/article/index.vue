      ...
      <el-form ref="form" :model="form" :rules="rules" label-width="80px">
        ...
        <el-form-item label="文章标签">
          <el-checkbox-group v-model="checkboxGroup">
            <el-checkbox :label="key+''"  v-for="(value,key) in labelAll" :key="key">{{value}}</el-checkbox>
          </el-checkbox-group>
        </el-form-item>
...
<script>
import { listArticle, getArticle, delArticle, addArticle, updateArticle, exportArticle } from "@/api/article/article"
import { listAll } from "@/api/article/label"
  ...
  methods: {
    ...
    getAll() {
      listAll().then(response=>{
        // console.log(response) 后台返回的数据格式
        this.labelAll = response.data
      })
    },
    // 取消按钮
    cancel() {
    ...
    handleUpdate(row) {
      ...
      getArticle(id).then(response => {
        ...
        if (response.data.labels) {
          this.checkboxGroup = response.data.labels.split(',')
        }
        this.open = true;
      });
    },
    ...
    submitForm() {
      this.$refs["form"].validate(valid => {
        if (valid) {
          this.form.labels = this.checkboxGroup.join(',')
    ...
    editRefine (row) {
      ...
      this.$confirm(text, "警告", {
        ...
        return updateArticle({ id, refine})
      }).then( _ => {
